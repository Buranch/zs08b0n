...
const findHighestInterceptedLocation = (sortedLocations, intersectedLocations) =>
  sortedLocations.find((location) => intersectedLocations
...
const route = (options, callback) => {
  ...
  const multi = db.multi();
  multi
    .zrangebyscore("location", '-inf', '+inf')
    .sinter(
        intersectBy
    )
